			...
			if($this->_request->isPost())
			{
				if($form->isValid($_POST))
				{
					...
					if(!empty($result)) { 
						...
						$mail_body = "Dear user,\nYou can change your password from here. \n".$config -> site_url.$this -> view -> url(array('module' => 'admin', 'controller' => 'login', 'action' => 'recoverpass', 'id' => $key))."\nIf link is broken please copy the link and paste into browser address bar.\nZend admin team
						";
						
						$transport = new Zend_Mail_Transport_Smtp($config -> smtp );
						$tr = new Zend_Mail_Transport_Smtp($config -> mail_from);
						$mail = new Zend_Mail('UTF-8');
						$mail->setBodyText($mail_body,'UTF-8',Zend_Mime::ENCODING_8BIT);
						$mail->setFrom($mailFrom, 'U.STYLE FASHION');
						$mail->addTo($result['email'],'password');					
						$mail->setSubject($this->translate->_('Password recovery'));
						$mail->send($transport);
	
						$this->view->message = $this->translate->_('Mail send to your email address for password recovery');
					} else {
						$this->view->message = $this->translate->_('Username is not correct');
					}
				}
